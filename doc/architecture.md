...
- UI: `PrysmVideoSurface` is headless video rendering. `PrysmVideoPlayer` composes surface plus default premium controls, with `surfaceBuilder` available for alternate render surfaces and engine-free widget tests. Progress and settings/picker internals are split into focused control parts to keep the main widget easier to maintain.
- Customization: `PrysmVideoCustomization` exposes granular builders for overlays, progress, menus, pickers, subtitles, gestures, keyboard handling, and TV focus without forcing callers to replace the whole controls layer.
